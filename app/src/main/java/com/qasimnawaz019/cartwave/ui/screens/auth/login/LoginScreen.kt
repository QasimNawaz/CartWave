...
fun LoginScreen(
    ...
    val loginState: NetworkUiState<LoginResponse> by viewModel.networkUiState.collectAsStateWithLifecycle()
    when (loginState) {
        ...
        is NetworkUiState.Success -> {
            if (isChecked) {
                (loginState as NetworkUiState.Success<LoginResponse>).data.token?.let {
                    viewModel.saveToken(
                        it
                    )
                }
    ...
    Column(
        ...
        Button(
            ...
            enabled = viewModel.usernameState.error == null && viewModel.passwordState.error == null
        ) {
            Text(text = "Login")
        }
        Spacer(modifier = Modifier.size(10.dp))
